...
Index = function ( ax, ay, bx, by, max, grow )
{
    ...
    this.insert = function ( geom, data )
    {
        ...
        if ( geom instanceof Shape )
            ...
            c = node.insert ( new Shape ( _.flatten ( geom ), data ) );
    ...
    this.query  = function ( geom )
    {
        ...
        console.log ( 'QUERY', geom, /* out, */ '#n', stat.nc, '#lvl', stat.lvl, '#s', out.length, stat.sc, ( 100 * out.length / stat.sc ).toFixed ( '2' ) + '% pass, time', Date.now () - time );
        return out;
    };
...
Node.prototype =
{
    insert : function ( shape )
    {
        var n;
            ...
            ////    Reject geometry that doesn't intersect with this cell.

        if ( shape.bx < this.ax || shape.ax > this.bx || shape.by < this.ay || shape.ay > this.by )
            return 0;
...
aabbTest = function ( ax, ay, bx, by, ex, ey, fx, fy )
{
...
lineIntersect = function ( ax, ay, bx, by, ex, ey, fx, fy, aline, bline, rpt )
{
    var ipx, ipy,
        a1, a2, b1, b2, c1, c2,
        d;
